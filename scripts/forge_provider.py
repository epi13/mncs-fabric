...
PROVIDER = {"id": "mncs-fabric-local", "name": "MNCS Fabric local validation", "identity": "mncs-fabric-local-v1", "version": "0.2"}
...
def run_validation() -> tuple[str, str, list[object]]:
    ...
    snapshot = json.loads((ROOT / "compat/mncs-execution-receipt-0.1.snapshot.json").read_text(encoding="utf-8"))
    if snapshot.get("unsupported_versions") != "fail-closed":
        return "FAIL", "compatibility snapshot is not fail-closed", []
    return "PASS", "Fabric suite, compilation, portable example, and compatibility snapshot passed", []
